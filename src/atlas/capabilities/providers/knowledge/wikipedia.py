    ...
    async def search(self, query: str, *, limit: int) -> list[KnowledgeItem]:
        try:
            ...
            r = await self._client.get(
                f"https://en.wikipedia.org/w/rest.php/v1/search/page?q={urllib.parse.quote(query)}&limit={limit}"
            )
            r.raise_for_status()
            ...
            for res in results[:limit]:
                ...
                r2 = await self._client.get(
                    f"https://en.wikipedia.org/w/api.php?action=query&prop=extracts&exintro&explaintext&format=json&titles={urllib.parse.quote(title)}"
                )
