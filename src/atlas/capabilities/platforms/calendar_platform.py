...
class CalendarPlatform:
    ...
    async def list_events(
        self, *, start: datetime, end: datetime, calendar_id: str | None = None, limit: int = 50
    ) -> list[CalendarEvent]:
        return await self._provider.list_events(calendar_id or self._default, start=start, end=end, limit=limit)

    async def search(self, query: str, *, limit: int = 20) -> list[CalendarEvent]:
        return await self._provider.search(query, limit=limit)

    async def free_busy(self, *, start: datetime, end: datetime, calendar_id: str | None = None) -> Availability:
        return await self._provider.free_busy(calendar_id or self._default, start=start, end=end)

    async def find_free_slots(self, *, start: datetime, end: datetime, min_minutes: int) -> list[FreeBusySlot]:
    ...
    async def commit(self, draft: EventDraft, correlation_id: CorrelationId, *, delete: bool = False) -> str | None:
        ...
        if not decision.approved:
            _log.info(
                "calendar.commit_denied",
                event_type="calendar",
                correlation_id=str(correlation_id),
                action=action,
                timed_out=decision.timed_out,
                unknown_attendees=len(unknown),
            )
            raise CapabilityDenied(f"{action} not approved" + (" (timed out)" if decision.timed_out else ""))

        if delete:
            await self._provider.delete_event(draft.calendar_id, draft.event_id or "")
            _log.info("calendar.deleted", correlation_id=str(correlation_id), event_id=draft.event_id)
            return None

        event_id = (
            await self._provider.update_event(draft) if draft.event_id else await self._provider.create_event(draft)
        )
        _log.info(
            "calendar.committed",
            event_type="calendar",
    ...
    def _render_preview(
        self, draft: EventDraft, action: str, unknown: list[object], conflicts: list[CalendarEvent]
    ) -> str:
        lines = [
        ...
        if unknown:
            lines.append("")
            lines.append(
                "⚠️ NEW CONTACTS (not in your known list): " + ", ".join(str(getattr(a, "email", a)) for a in unknown)
            )
        if conflicts:
            lines.append("")
            lines.append("⚠️ CONFLICTS with: " + "; ".join(f"{c.title} ({c.when.render()})" for c in conflicts))
