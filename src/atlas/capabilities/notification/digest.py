...
class DigestEngine:
    def __init__(self, *, queue: NotificationQueue, gateway: ModelGateway, ids: IdGenerator, clock: Clock) -> None:
        self._queue = queue
        self._gw = gateway
        self._ids = ids
        self._clock = clock
    ...
    async def build(self, correlation_id: str) -> Notification | None:
        ...
        resp = await self._gw.complete(
            ModelRequest(
                ...
                system="Summarize this digest crisply, keep all headers.",
                prompt=raw,
        ...
        return Notification(
            ...
            kind=NotificationKind.SYSTEM_HEALTH,
            priority=NotificationPriority.LOW,
            title=f"ATLAS digest ({len(batched)} items)",
            body=resp.text,
