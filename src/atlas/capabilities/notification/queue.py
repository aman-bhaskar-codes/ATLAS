...
class NotificationQueue:
    ...
    async def enqueue(self, n: Notification, *, not_before_iso: str | None = None, digest: bool = False) -> bool:
        if n.dedup_key:
    ...
    async def cancel(self, notification_id: str) -> None:
        await self._db.conn.execute(
            "UPDATE notif_queue SET state='cancelled' WHERE id=? AND state='pending'", (notification_id,)
        )
        await self._db.conn.commit()
