...
class AvailabilityEngine:
    ...
    def free_slots(
        self, events: list[CalendarEvent], *, window_start: datetime, window_end: datetime, duration: timedelta
    ) -> list[FreeSlot]:
    ...
    def _clip_to_working(self, start: datetime, end: datetime, duration: timedelta) -> list[FreeSlot]:
        out: list[FreeSlot] = []
        cur = start
        while end - cur >= duration:
            if self._t.is_working_time(cur) and self._t.is_working_time(cur + duration - timedelta(minutes=1)):
