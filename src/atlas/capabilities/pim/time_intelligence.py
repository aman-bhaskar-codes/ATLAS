...
class TimeIntelligence:
    def __init__(self, clock: Clock, working_hours: WorkingHours, holidays: frozenset[str] = frozenset()) -> None:
        self._clock = clock
    ...
    def next_working_slot(self, after: datetime, duration: timedelta) -> datetime:
        ...
        for _ in range(14 * 24):  # bounded search, 2 weeks of hours
            if self.is_working_time(cur) and self.is_working_time(cur + duration - timedelta(minutes=1)):
                return cur
