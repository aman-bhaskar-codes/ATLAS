...
class Orchestrator:
    ...
    async def _save_trajectory(
        ...
        *,
        ...
        """
        from atlas.memory.trajectory import ActionRecord, Trajectory
        ...
        trajectory = Trajectory(
            ...
            plan_steps=tuple(
                getattr(step, "intent", str(step))
                for step in getattr(plan, "steps", [])
            ),
            risk_level=getattr(plan, "risk", "low").value if hasattr(getattr(plan, "risk", None), "value") else "low",
