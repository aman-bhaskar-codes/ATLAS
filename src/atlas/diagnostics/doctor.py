...
async def _verify_encrypted_store(atlas: Atlas) -> tuple[bool, str]:
    ...
    try:
        ...
        detail = f"{count} encrypted secret rows verified" if ok else f"vault verification failed at row {count}"
        return ok, detail
...
async def run_doctor(atlas: Atlas, *, verify_manifest_only: bool = False) -> list[CheckResult]:
    ...
    report = verify_manifest(atlas.manifest, _REGISTERED_TOOLS, set(KNOWN_CONSTRAINTS), set(KNOWN_MATCHERS))
    if report.matcher_gaps:
        results.append(
            CheckResult("manifest.matchers", "fail", f"unimplemented safety matchers: {report.matcher_gaps}")
        )
    elif report.unmatched_constraints:
        results.append(
            CheckResult("manifest.constraints", "fail", f"unknown constraints: {report.unmatched_constraints}")
        )
    else:
        results.append(CheckResult("manifest", "pass", f"v{atlas.manifest.version}, {len(atlas.manifest.rules)} rules"))
    if report.orphan_rules:
        results.append(CheckResult("manifest.orphans", "warn", f"rules for not-yet-built tools: {report.orphan_rules}"))
    ...
    if docker_ok:
        ...
        results.append(CheckResult("sandbox.runtime", "warn", "Docker unavailable; native development fallback active"))
    else:
        results.append(CheckResult("sandbox.runtime", "fail", "Docker sandbox required outside development"))
    ...
    results.append(
        CheckResult(
            "directories", "pass" if data_dir.exists() else "warn", f"data_dir={data_dir} exists={data_dir.exists()}"
        )
    )

    # secrets (presence only — never print values)
    push = "configured" if atlas.settings.ntfy_topic else "absent (CLI-only confirmations)"
    results.append(CheckResult("secrets.ntfy", "pass" if atlas.settings.ntfy_topic else "warn", push))
    ...
    results.append(CheckResult("identity.encryption", "pass" if vault_ok else "fail", vault_detail))
    ...
    results.append(
        CheckResult("future.providers", "pass", "provider adapter layer present (cloud disabled in Phase 1)")
    )
    return results
