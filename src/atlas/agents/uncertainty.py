...
class UncertaintyQuantifier:
    ...
    async def quantify_uncertainty(
        ...
        self._stats["avg_epistemic"] = (self._stats["avg_epistemic"] * (n - 1) + epistemic) / n
        self._stats["avg_aleatoric"] = (self._stats["avg_aleatoric"] * (n - 1) + aleatoric) / n
    ...
    async def calibrated_prediction(
        ...
        uncertainty = UncertaintyEstimate(
            ...
            uncertainty_type=UncertaintyType.TOTAL,
            confidence_interval=self._build_confidence_interval(calibrated_conf, epistemic, aleatoric),
            epistemic_uncertainty=epistemic,
            aleatoric_uncertainty=aleatoric,
...
Output JSON: {{"aleatoric_uncertainty": 0.0-1.0}}"""
    ...
    def _calibrate_confidence(
        ...
        calibrated = self._calibration_params["slope"] * raw_confidence + self._calibration_params["intercept"]
    ...
    def _calculate_calibration_score(
        ...
        bin_samples = [
            (conf, correct) for conf, correct in self._calibration_history if bin_idx <= conf / bin_size < bin_idx + 1
        ]
    ...
    def _update_calibration(self) -> None:
        ...
        sum_xy = sum(conf * (1.0 if correct else 0.0) for conf, correct in self._calibration_history)
        ...
        alpha = 0.1
        self._calibration_params["slope"] = self._calibration_params["slope"] * (1 - alpha) + slope * alpha
        self._calibration_params["intercept"] = self._calibration_params["intercept"] * (1 - alpha) + intercept * alpha
