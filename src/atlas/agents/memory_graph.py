  ...
  "facts": [
    ...
    def _find_similar_relation(
        ...
        for relation in relations.values():
            if (
                relation.source_entity == new_relation.source_entity
                and relation.target_entity == new_relation.target_entity
                and relation.relation_type == new_relation.relation_type
            ):
                return relation
        return None
    ...
    def _prune_low_confidence(self) -> None:
        ...
        # Prune entities
        self._entities = {eid: ent for eid, ent in self._entities.items() if ent.confidence >= self._min_confidence}

        # Prune relations
        self._relations = {rid: rel for rid, rel in self._relations.items() if rel.confidence >= self._min_confidence}

        # Prune temporal facts
        self._temporal_facts = [f for f in self._temporal_facts if f.confidence >= self._min_confidence]
    ...
    def _calculate_overall_confidence(self) -> float:
        ...
        all_confidences = [e.confidence for e in self._entities.values()] + [
    ...
    def get_statistics(self) -> dict[str, Any]:
        ...
        return {
            **self._stats,
            ...
            "entities_by_type": {t.value: sum(1 for e in self._entities.values() if e.type == t) for t in EntityType},
            "relations_by_type": {
                t.value: sum(1 for r in self._relations.values() if r.relation_type == t) for t in RelationType
            },
        }
