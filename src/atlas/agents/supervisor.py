...
class SupervisorAgent(BaseAgent):
    ...
    async def execute(
        ...
        *,
        ...
        try:
            ...
            if errors:
                _log.warning(
                    "supervisor.dag_invalid", event_type="agents", correlation_id=correlation_id, errors=errors
                )
            ...
            _log.error("supervisor.failed", event_type="agents", correlation_id=correlation_id, error=repr(exc))
            return SubTaskResult(
                subtask_id=subtask_id,
                agent_type=self.agent_type,
                ok=False,
    ...
    async def orchestrate(
        ...
        for batch in dag.topological_batches():
            ...
            for subtask in batch:
                dep_results = {
                    dep_id: completed[dep_id].output for dep_id in subtask.dependencies if dep_id in completed
                }
    ...
    async def _dispatch(
        ...
        if not self._registry.has(agent_type):
            _log.warning(
                "supervisor.unknown_agent_type", event_type="agents", agent_type=agent_type, fallback="general"
            )
