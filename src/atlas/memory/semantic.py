...
class SemanticMemory:
    ...
    async def _on_task_event(self, event: Event) -> None:
        ...
        try:
            ...
            if committed > 0:
                _log.info("semantic.facts_committed", event_type="memory", task_id=event.task_id, count=committed)

        except Exception as exc:
            _log.error("semantic.extraction_error", event_type="memory", task_id=event.task_id, error=str(exc))

    async def _extract_facts_fast(self, event: Event) -> list[tuple[str, FactKind, float, float]]:
        """
    ...
    async def extract_facts_llm(
        self, task_summary: str, outcome: str, max_facts: int = 3
        ...
        _log.debug(
            "semantic.llm_extraction_skipped", event_type="memory", reason="Not yet implemented - rule-based sufficient"
        )
        return []

    async def get_recent_facts(
        self, kind: FactKind | None = None, limit: int = 20, min_confidence: float = 0.5
    ) -> list[SemanticFact]:
        """Get recent facts with optional filtering (< 30 ms DB, < 1 ms cached)."""
        cache_key = self._fact_cache.make_key(kind.value if kind else None, min_confidence, limit)
    ...
    async def get_facts_by_confidence(self, min_confidence: float = 0.8, limit: int = 50) -> list[SemanticFact]:
    ...
    async def supersede(self, old_id: str, new_text: str, *, confidence: float) -> str:
        ...
        cur = await self._db.conn.execute("SELECT kind, salience, version FROM semantic_facts WHERE id=?", (old_id,))
