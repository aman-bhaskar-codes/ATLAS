...
class KnowledgeStore:
    ...
    async def ingest_document(
        ...
        """
        _log.info(
            "knowledge_store.ingest_started",
            event_type="memory",
            path=str(file_path),
            type=source_type
        )
        ...
        if existing:
            _log.info(
                "knowledge_store.duplicate_detected",
                event_type="memory",
                doc_id=existing.id,
                path=str(file_path)
            )
        ...
        async for chunk in self._parse_document_streaming(
            file_path, doc_id, source_type, metadata or {}
        ):
            chunks.append(chunk)
        ...
        for chunk in chunks:
            task = asyncio.create_task(
                self._embed_and_index_chunk(chunk, progress_callback)
            )
        ...
        _log.info(
            "knowledge_store.ingest_complete",
            event_type="memory",
            doc_id=doc_id,
            chunks=len(chunks)
        )
    ...
    async def _parse_document_streaming(
        ...
        if source_type == "markdown" or source_type == "txt":
            ...
            _log.warning(
                "knowledge_store.unsupported_type",
                event_type="memory",
                type=source_type
            )
            return
    ...
    async def _embed_and_index_chunk(
        ...
        try:
            ...
            if progress_callback:
                await progress_callback({
                    "status": "indexing",
                    "chunk_indexed": chunk.chunk_index + 1,
                    "total_chunks": chunk.total_chunks
                })
                
        except Exception as exc:
            _log.error(
                "knowledge_store.chunk_error",
                event_type="memory",
                chunk_id=chunk.chunk_id,
                error=str(exc)
            )
    ...
    async def _save_document(self, doc: Document) -> None:
        ...
        _log.info(
            "knowledge_store.document_saved",
            event_type="memory",
            doc_id=doc.id,
            title=doc.title
        )
    ...
    async def _mark_indexed(self, doc_id: str) -> None:
        ...
        _log.info(
            "knowledge_store.marked_indexed",
            event_type="memory",
            doc_id=doc_id
        )
    ...
    async def delete_document(self, doc_id: str) -> None:
        ...
        cur = await self._db.conn.execute(
            "SELECT chunk_id, embedding_id FROM knowledge_chunks WHERE document_id = ?",
            (doc_id,)
        )
        ...
        _log.info(
            "knowledge_store.document_deleted",
            event_type="memory",
            doc_id=doc_id,
            chunks_deleted=len(rows)
        )
