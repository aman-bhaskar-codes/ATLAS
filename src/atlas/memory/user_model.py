    ...
    async def _on_feedback_event(self, event: Event) -> None:
        ...
        try:
            ...
            preferences = await self._infer_preferences(
                rating=rating, comment=comment, original=original_output, edited=edited_output
            )
            ...
            _log.error("user_model.feedback_error", event_type="memory", error=str(exc))

    async def _infer_preferences(
        self, rating: int | None, comment: str | None, original: str | None, edited: str | None
    ) -> dict[str, str]:
        """
        ...
        if rating == -1:
            ...
            _log.debug("user_model.negative_feedback", event_type="memory", comment=comment[:100] if comment else None)

        return preferences

    async def _get_section(self, section: str) -> str:
        """Get current content of a section."""
        cur = await self._db.conn.execute("SELECT content FROM user_model WHERE section = ?", (section,))
        row = await cur.fetchone()
        return row["content"] if row else ""

    async def _merge_preferences(self, current: str, new_prefs: dict[str, str]) -> str:
    ...
    async def set_section(self, section: str, content: str) -> None:
        ...
        if self._bus:
            try:
                ...
                _log.warning("user_model.broadcast_error", event_type="memory", error=str(exc))
