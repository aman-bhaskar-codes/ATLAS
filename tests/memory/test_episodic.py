...
async def test_episodic_record_and_recent(mem_db) -> None:  # type: ignore
    ...
    ep = Episode(
        correlation_id="c1", ts=clock.now(), kind=EpisodeKind.ACTION, role="agent", content="did a thing", tokens=10
    )
...
async def test_episodic_unconsolidated(mem_db) -> None:  # type: ignore
    clock = SystemClock()
    epi = EpisodicMemory(mem_db, clock)
    ep = Episode(correlation_id="c2", ts=clock.now(), kind=EpisodeKind.MESSAGE, role="user", content="hello", tokens=5)
    eid = await epi.record(ep)
