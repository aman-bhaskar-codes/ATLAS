...
class FakeVectors:
    async def upsert(self, ref, text, embedding) -> None:
        pass  # type: ignore

    async def query(self, embedding, k) -> None:
        return []  # type: ignore
...
class FakeEmbedder:
    async def embed(self, text) -> None:
        return [0.1, 0.2, 0.3]  # type: ignore
...
async def test_semantic_versioning(sem_db) -> None:  # type: ignore
    ...
    fid = await sem.add_fact("likes apple", FactKind.PREFERENCE, confidence=1.0, salience=0.5, sources=[1])
