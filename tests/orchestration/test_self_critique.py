...
class ScriptedGateway:
    ...
    async def complete(self, req: ModelRequest) -> ModelResponse:
        ...
        return ModelResponse(
            text=text, target=ModelTarget.LOCAL_FAST, model="fake", cost=TokenCost(input_tokens=1, output_tokens=1)
        )
...
def _hook(gateway: ScriptedGateway, tier: Tier) -> SelfCritique:
    ...
    return SelfCritique(
        gateway=gateway,
        estimator=est,
        parser=ResponseParser(),  # type: ignore
        validator=OutputValidator(),
        correlation_id_provider=lambda: CorrelationId("c"),
    )


_TOOL_ACTION = Action(step=1, kind="tool_call", tool="filesystem", operation="delete", args={"path": "/x"})
...
async def test_benign_action_skips_critique() -> None:
    ...
    out = await hook.critique(Action(step=1, kind="tool_call", tool="filesystem", operation="read", args={}), "ctx")
