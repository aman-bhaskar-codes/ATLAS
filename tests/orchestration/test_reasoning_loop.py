...
class FakeGateway:
    ...
    async def complete(self, req: ModelRequest) -> ModelResponse:
        ...
        return ModelResponse(
            text=text, target=ModelTarget.LOCAL_FAST, model="fake", cost=TokenCost(input_tokens=5, output_tokens=5)
        )
...
class FakeRecorder:
    async def record_thought(self, *a: Any) -> None:
        pass

    async def record_action(self, *a: Any) -> None:
        pass

    async def record_observation(self, *a: Any) -> None:
        pass


class FakeEvents:
    async def emit(self, **k: Any) -> None:
        pass
...
async def test_tool_then_final() -> None:
    gw = FakeGateway(
        [
            ...
            '{"thought":"done","confidence":0.9,"action":{"kind":"final_answer","final_text":"here it is"}}',
        ]
    )
    disp = FakeDispatcher()
