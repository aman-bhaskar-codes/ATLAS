...
async def test_knowledge_platform_routing() -> None:
    ...
    official_provider.search.return_value = [
        KnowledgeItem(
            title="Official News",
            snippet="The mock news",
            provenance=Provenance(
                provider="rss:mock", source_kind=SourceKind.OFFICIAL, uri="http://mock", retrieved_ts=datetime.now(UTC)
            ),
        )
    ]
    ...
    platform = KnowledgePlatform(
        ...
        web=[],
        memory_source=AsyncMock(),
        parametric=AsyncMock(),
    )

    router.classify.return_value = KnowledgeIntent.LIVE
    gateway.complete.return_value = type("R", (), {"text": "The mock news says this."})()
