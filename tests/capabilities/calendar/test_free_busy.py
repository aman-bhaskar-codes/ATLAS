...
def _event(start_h: int, end_h: int, eid: str = "x") -> CalendarEvent:
    return CalendarEvent(
        id=eid, calendar_id="primary", title="busy", when=EventTime(start_dt=_dt(start_h), end_dt=_dt(end_h))
    )
...
class FakeProvider:
    ...
    async def update_event(self, draft: object) -> str:
        return "x"
...
class DenyNotify:
    async def request_approval(self, req: object, channels: tuple[str, ...]) -> object:
        ...
        return ApprovalDecision(request_id=req.id, approved=False, decided_ts=datetime.now(UTC))
...
def _platform() -> CalendarPlatform:
    return CalendarPlatform(
        ...
        notifications=DenyNotify(),  # type: ignore[arg-type]
        ids=FakeIds(),
        known=KnownContacts(set()),  # type: ignore[arg-type]
        approval_channels=(),
    )
...
async def test_find_free_slots_returns_gaps() -> None:
    ...
    async def _fake_free_busy(cal_id: str, *, start: datetime, end: datetime) -> Availability:
        return Availability(
            ...
            window_start=start,
            window_end=end,
            busy=(FreeBusySlot(start=_dt(10), end=_dt(11)), FreeBusySlot(start=_dt(14), end=_dt(15))),
        )
...
async def test_back_to_back_busy_yields_no_slot() -> None:
    ...
    async def _full(cal_id: str, *, start: datetime, end: datetime) -> Availability:
        return Availability(
            calendar_id=cal_id, window_start=start, window_end=end, busy=(FreeBusySlot(start=_dt(9), end=_dt(17)),)
        )
