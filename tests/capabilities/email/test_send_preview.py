...
def _platform(approved: bool, known: tuple[str, ...] = ()) -> EmailPlatform:
    return EmailPlatform(
        ...
        notifications=ApproveNotify(approved),  # type: ignore
        ids=Ids(),
        known_contacts=set(known),  # type: ignore
        approval_channels=("telegram:primary",),
    )


_DRAFT = EmailDraft(to=(EmailAddress(email="boss@corp.com"),), subject="Q3 Report", body_text="Attached.")
